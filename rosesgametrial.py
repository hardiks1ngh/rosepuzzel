...
def randomiseDiceImages(dice_images):
    ...
    shuffled_dice_images = {}
    for i in range(6):
        shuffled_dice_images[dice_items[i][0]] = dice_items[random.randint(0, 5)][1]
    shuffled_dice_images = dict(shuffled_dice_images)
...
class petalsGame:
    ...
    def randomiseDiceArr(self, diceArrToUse=[]):
        diceArrToUse = randomiseDiceImages(dice_images)
        print(diceArrToUse)
        for key in diceArrToUse:
            if key == 3:
